...
//                container.getEndpointConfiguration(LocalStackContainer.Service.S3))
//            .withCredentials(container.getDefaultCredentialsProvider())
//            .build();
        AmazonS3 s3 = AmazonS3ClientBuilder.standard()
            .withEndpointConfiguration(
                new EndpointConfiguration(container.getEndpoint().toString(),
                    container.getRegion().toString()))
            .withCredentials(
        ...
        List<String> results = IOUtils.readLines(s3.getObject(bucketName, key).getObjectContent(),
            "utf-8");
